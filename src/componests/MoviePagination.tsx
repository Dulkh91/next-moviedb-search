...
const contentType = isRatedPage? 'rated': (query && isSearchPage? 'search': 'home') // 'home' for discover


// Get the current page from URL. If not present, use the cached page for the current content type.
const currentPageFromUrl = Number(searchParams.get("page") || "1");

const { data, isValidating } = useMovie(query, String(currentPageFromUrl), contentType === 'search' || contentType === 'home' ? contentType : 'discover'); 
...
if (isValidating || isLoadingRated) { // Use isValidating for data refresh, isLoadingRated for rated data initial load
  return 
}
